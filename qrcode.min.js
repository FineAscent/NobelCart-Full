...
(function(global){
  ...
  var QRRSBlock = (function(){ var RS_BLOCK_TABLE = [
    ...
    var _this={}; _this.getRSBlocks=function(typeNumber,errorCorrectLevel){ var rsBlock = RS_BLOCK_TABLE[(typeNumber-1)*4 + errorCorrectLevel]; if(rsBlock.length==3) return [ QRRSBlock(rsBlock[0], rsBlock[1]) ]; else { var list=[]; for(var i=0;i<rsBlock.length;i+=3){ var count=rsBlock[i]; var total=rsBlock[i+1]; var data=rsBlock[i+2]; for(var j=0;j<count;j++) list.push(QRRSBlock(total,data)); } return list; } };
    return _this;
  })();
